...
const recordScamIntelligence = tool(
  ...
  },
  {
    ...
    description: "CRITICAL: Only call this tool when the scammer EXPLICITLY provides a UPI ID, bank account number, or URL in the chat. Do NOT call this tool if you are still waiting for them to send it or if you are just asking for it.",
    schema: z.object({
      type: z.enum(["upiId", "bankAccount", "phishingLink"]),
      value: z.string().describe("The exact string provided by the scammer"),
    }),
  }
);
